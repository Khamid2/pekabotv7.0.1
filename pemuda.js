...
NOTE : Untuk Mendapatkan Limit Bisa Lewat Naik Level Di Group Atau Buy limit.`, text, { quoted : mek})
			found = true 
			}
		}
			if (found === false) {
...
┕━━━━━━━━━━━━┈ ❋ཻུ۪۪⸙
*「 Peka Bot 」*`
let buff = await getBuffer(`http://hadi-api.herokuapp.com/api/card/verify?nama=${encodeURI(pushname)}&member=${daftar.length}&seri=${serialUser}&pp=${ppimg}&bg=${ppimg}`)
pemuda.sendMessage(from, buff, MessageType.image, {caption: dpter, contextInfo: {"mentionedJid": [sender]}})
...
╚══════════════════✿
`
pemuda.sendMessage(from, menu, text, { quoted: ftrol, thumbnail: fs.readFileSync('./pemuda_1.jpg'), contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: `${jmn} - ${week} ${weton} - ${calender}`,body:"",mediaType:"2",thumbnail:ofrply,mediaUrl:`https://youtu.be/7LefYmD9n84`}}})
break
case 'listmenu':
if (!isDaftar) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: mek})
 listMsg = {
