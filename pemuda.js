...
try {
        ...
        mess = {
			...
			only: {
				...
				ownerG: 'fitur khusus owner bot dalam grub!',
				ownerB: 'Fitur khusus owner bot!',
				admin: 'Hanya admin grub yang dapat menggunakan fitur ini!',
				Badmin: 'Fitur ini hanya dapat digunakan setelah bot menjadi admin!'
			},
		pk: {
			daftarsek: '──「 DAFTAR 」──\nMaaf Kak Kamu belum Terdaftar Sebagai Teman Peka, Untuk Berteman Dengan Peka Bot, kirim pesan dengan cara, \n*.daftar*\n──「 Peka Bot 」──',
			}
		}
...
//CHECK LIMIT
          const checkLimit = (sender) => {
			...
			if (limitCounts <= 0) return pemuda.sendMessage(from,`Limit request anda sudah habis\n`, text, {quoted: mek})
			pemuda.sendMessage(from, `
「 ❗ 」Limit Count
Sisa Limit Anda : ${limitCounts}

NOTE : Untuk Mendapatkan Limit Bisa Lewat Naik Level, memainkan game dan membeli limit.`, text, { quoted : mek})
			found = true 
			}
		}
			if (found === false) {
			...
			_limit.push(obj)
			fs.writeFileSync('./database/limit.json', JSON.stringify(_limit))
			pemuda.sendMessage(from, `limit anda : ${limitCounts}`, text, { quoted : mek})
			}
		} 
//-
//LIMIT
           const isLimit = (sender) =>{ 
              ...
              if (limits >= limitawal ) {
              	  position = true
                    pemuda.sendMessage(from, pklvl.limitend(pushname), text, {quoted: mek})
                    return true
              } else {
              	_limit
                  position = true
...
//-
 ...
 //Balas limit habis
const textLimit = `「 ❗ 」Maaf ${pushname} Limit Kamu Hari ini Sudah Habis
NOTE : Untuk Mendapatkan Limit Bisa Lewat Naik Level, memainkan game dan membeli limit`
             tombolLimit = [{buttonId: `${prefix}Gamemenu`, buttonText: {displayText: 'Game'}, type: 1},{buttonId: `${prefix}carabelilimit`, buttonText: {displayText: 'buylimit'}, type: 1}]
             wadahLimit = {
                contentText: textLimit,
                footerText: 'Klik tombol dibawah ini',
                buttons: tombolLimit,
                headerType: 4
             }
...
case 'daftar':
case 'verify':
	        if (isDaftar) return reply(mess.only.wsdaftar)
	        const serialUser = bikinSerial(20)
	         try {
								ppimg = await pemuda.getProfilePicture(`${sender.split('@')[0]}@c.us`)
								} catch {
...
ya = await getBuffer(`http://hadi-api.herokuapp.com/api/card/verify?nama=${encodeURI(pushname)}&member=${daftar.length}&seri=${serialUser}&pp=${ppimg}&bg=${ppimg}`)
py = await pemuda.prepareMessage(from, ya, image)
sendBtnVeryy = {
imageMessage: py.message.imageMessage,
contentText:`${dpter}`,
footerText:'Terimakasih Sudah Mendaftar\n*_© Created by Khamid_*', 
buttons: gbutsan,
headerType: 4
}
pemuda.sendMessage(from, sendBtnVeryy, MessageType.buttonsMessage, {quoted:mek, contextInfo: {"mentionedJid": [sender]}})
console.log(color('[REGISTER]'), color(time, 'yellow'), 'Serial:', color(serialUser, 'cyan'), 'in', color(sender || groupName))
break
case 'help':
        if (!isDaftar) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: mek})
        ptod = "6282322888788@s.whatsapp.net"
        stod = `${sender}`
        uwu = '```'
       stst = await pemuda.getStatus(`${sender.split('@')[0]}@c.us`)
				stst = stst.status == 401 ? '' : stst.status
			
       menuhelp = `*Halo ${pushname}*
...
case 'helpp':
       ...
       menu2 = `*Halo ${pushname}*`
sendButLocation(from, `${menu2}`, `${uwu2}Created by @Khamid${uwu2}`, {jpegThumbnail:ofrply}, [{buttonId:`${prefix}allmenu`,buttonText:{displayText:'MENU ALL📒'},type:1},{buttonId:`${prefix}owner`,buttonText:{displayText:'DEVELOPER👤'},type:1}], {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: `${jmn} - ${week} ${weton} - ${calender}`,body:"",mediaType:"2",thumbnail:ofrply,mediaUrl:`https://youtu.be/7LefYmD9n84`}}})
break
case 'allmenu':
case 'menu':
        if (!isDaftar) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: mek})
...
╚══════════════════✿
`
butt = [{buttonId: `${prefix}donasi`, buttonText: {displayText: 'Donasi'}, type: 1},{buttonId: `${prefix}owner`, buttonText: {displayText: 'owner'}, type: 1}]
             gbuttonan = {
...
pemuda.sendMessage(from, menu, text, { quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: `${jmn} - ${week} ${weton} - ${calender}`,body:"",mediaType:"2",thumbnail:ofrply,mediaUrl:`https://youtu.be/7LefYmD9n84`}}})
break
...
pemuda.sendMessage(from, listMsg, MessageType.listMessage, {contextInfo: { mentionedJid: [stod]},quoted:mek})
break
case 'ownermenu':
if (!isDaftar) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: mek})
menu = `⬣ 「 \`\`\`MENU OWNER\`\`\` 」
...
imageMessage = (await pemuda.prepareMessageMedia({url:res.all[0].image},'imageMessage',{thumbnail:Buffer.alloc(0)})).imageMessage
...
*Pembelian Premium yang disertai SewaBot hanya akan membayar 20K (Diskon 5K)*`
               ...
               imageMsg = (await pemuda.prepareMessageMedia(fs.readFileSync(`./media/sewa.jpg`), 'imageMessage', {thumbnail: fs.readFileSync(`./media/sewa.jpg`)})).imageMessage

               buttonsMessage = {
               contentText: `${menu}`,
               footerText: 'List Sewa\n*• Rp. 15.000,/Perbulan*\n*•Rp. 30.000,/permanen*\n\nKetik Di Bawah Untuk Nyewa BOT', imageMessage: imageMsg,
               buttons: buttons,
               headerType: 4
}
...
case 'developer':
		case 'author':
pemuda.sendMessage(from, {displayname: `${NamaOwner}ツ`, vcard: vcard1}, MessageType.contact, {quoted: mek, contextInfo: { forwardingScore: 508, isForwarded: true }})
                pemuda.sendMessage(from, 'Tuh nomer owner ku😎 jangan dispam yak',MessageType.text, {quoted: mek})
                break
case 'addcmd': 
case 'setcmd':
if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
...
break
		...
		case 'sc':
		case 'sourcecode':
		pemuda.sendMessage(from, { text: "Kunjungi cyberklik.com", matchedText: 'https://cyberklik.com', description: "", title: "don't click here !!!", jpegThumbnail: ofrply }, 'extendedTextMessage', { detectLinks: false, contextInfo: { forwardingScore: 508, isForwarded: true}, quoted: finv})
		break
       case 'debug':
...
await pemuda.sendMessage(from, buMess, MessageType.buttonsMessage, {quoted: ftrol})
break
				case 'public':
				if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
			publik = true
...
break
                case 'ytmp44444444':
						...
						break
						case 'ytsearch':
            if (!args.length) return reply('Mau nyari apa kak,  example: .ytsearch aku dan kamu')
            try {
            	reply(mess.wait)
   			...
   			var mulaikah = aramat[0].url							
                  try {
                    ...
                    .then((res) => {
                        ...
                        const captions = `.•♫•♬• *PLAYING MUSIC* .•♫•♬•\n\n📜𝙏𝙞𝙩𝙡𝙚: ${title}\n🎁𝙏𝙮𝙥𝙚 : mp3\n𝙁𝙞𝙡𝙚 𝙎𝙞𝙯𝙚 : ${filesizeF}\n🖇𝙇𝙞𝙣𝙠 ${a.data}\n\n_Tunggu sebentar lagu sedang dikirim_`
				...
				case 'sticker':
					...
					case 's':
					if (!isDaftar) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: mek})
					if (isLimit(sender)) return pemuda.sendMessage(from, wadahLimit, MessageType.buttonsMessage, { quoted: mek})
						if (isMedia && !mek.message.videoMessage || isQuotedImage) {
							const encmedia = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
...
case 'bagaimanakah':
				...
				pemuda.sendMessage(from, 'Pertanyaan : *'+bagaimanakah+'*\n\nJawaban : '+ mana, text, { quoted: mek })
...
case 'user':
				...
				for (let premm of _registered) {
					teks += `~> @${premm.split('@')[0]}\n`
					}
					teks += `Total : ${_registered.length}`
				pemuda.sendMessage(from, teks.trim(), extendedText, {quoted: mek, contextInfo: {"mentionedJid": _registered}})
				break
case 'listuser':
case 'lispengguna':
			if (!isOwner && !mek.key.fromMe) return reply(mess.only.ownerB)
reply(`*「 Total Pengguna 」* \n\n *${daftar.length}* Orang`)
break
case 'ppcp': 
case 'ppcouple':
			anu = await fetchJson(`https://ziyy.herokuapp.com/api/ppcouple?apikey=xZiyy`) 
			cowo = await getBuffer(anu.result.cowo_image)
			pemuda.sendMessage(from, cowo, image, {caption: 'Ini Untuk »  *Cowoknya*'})
       cewe = await getBuffer(anu.result.cewe_image)
		    pll = `Ini Untuk » *Ceweknya*`
   sendButImage(from, pll, `Peka bot© 2021`, cewe, [                      
          {
...
case 'ytdl':
            if (!isDaftar) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: mek})
             ...
             if (args.length < 1) return reply(`Kirim perintah *${prefix}ytdl judul lagu`)
             reply(mess.wait)
             let yut = await yts(c)
...
Link Channel : ${yut.all[0].author.url}`
     ...
     case 'ytmp4':
              if (!isDaftar) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: mek})
             //if (!isRegister) return reply(mess.only.regis)      	     
             if (args.length === 0) return reply('linknya mana?\n\ncontoh\n.ytmp4 https://youtu.be/_2TN3AkBhE0')            
             res = await ytv(`${body.slice(7)}`).catch(e => {
...
case 'leveling':
				...
				break				
				case 'level':
				if (!isDaftar) return reply(pklvl.noregis())
                if (!isLevelingOn) return reply(pklvl.lvlnoon())
                const userLevel = getLevelingLevel(sender)
                const userXp = getLevelingXp(sender)
                if (userLevel === undefined && userXp === undefined) return reply(pklvl.lvlnul())
                const requiredXp = 2000 * (Math.pow(2, userLevel) - 1)
                resul = `┏━━❉ LEVEL ❉━━\n┣⊱ Nama : ${pushname}\n┣⊱ Nomor : wa.me/${sender.split("@")[0]}\n┣⊱ User XP :  ${userXp}/${requiredXp}\n┣⊱ User Level : ${userLevel}\n┗━━━━━━━━━━━━`
                pemuda.sendMessage(from, resul, text, { quoted: mek})
                .catch(async (err) => {
                console.error(err)
                await reply(`Error!\n${err}`)
                })
...
case 'dompet':
				...
				reply(pklvl.uangkau(pushname, sender, kantong))
				break
case ' transfer':
				case 'transfer':
				case 'tf':
				if (!isDaftar) return sendButMessage (from, daftar1, daftar2, daftar3, { quoted: mek})
				if (!q.includes('|')) return  reply(pklvl.wrongf())
				...
				const totalcoin = koinPerlimit * payout
				if ( checkATMuser(sender) <= totalcoin) return reply(`MAAF UANG KAMU BELUM CUKUP, JIKA INGIN MELIHAT UANG KETIK ${prefix}dompet`)
				if ( checkATMuser(sender) >= totalcoin ) {
					confirmATM(sender, totalcoin)
...
case 'ocr': 
              ...
              pemuda.sendMessage(from, ini_buffer, image, { quoted: freply, caption: '*+* ```Tebak Anime```\n\n• *Petunjuk* :'+kisi_kisi+'\n• *Waktu* : 30s' }).then(() => {
...
pemuda.sendMessage(from, ini_buffer, image, { quoted: mek, caption: 'Silahkan jawab soal berikut ini\n\nPetunjuk :tebak sendirilah kontol\nWaktu : 30s' }).then(() => {
...
*Silahkan pilih media yang akan di download*
`
buttons = [{buttonId:`${prefix}play2 ${q}`,buttonText:{displayText:'🎥VIDEO'},type:1},{buttonId:`${prefix}ytmp3 ${q}`,buttonText:{displayText:'🎵AUDIO'},type:1}]

imageMessage = (await pemuda.prepareMessageMedia({url:res.all[0].image},'imageMessage',{thumbnail:Buffer.alloc(0)})).imageMessage

buttonsMessage = {contentText: thumbInfoyt,footerText:'Silahkan Pilih Jenis File Dibawah Ini',imageMessage,buttons,headerType:4}
...
Giliran = @${tty.er1.split('@')[0]}`
 pemuda.sendMessage(from, ucapan, text, {quoted: mek, contextInfo:{mentionedJid: [tty.er1,tty.er2]}})
     ...
     pemuda.sendMessage(`${settings.NomorOwner}@s.whatsapp.net`, `─────「 *ALERT-ERROR* 」─────\n\n\`\`\`${e}\`\`\`\n\n────────────────────`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "Developer Bitch Boot",body:"",previewType:"PHOTO",thumbnail:fs.readFileSync('./pemuda_1.jpg'),sourceUrl:"https://youtu.be/7LefYmD9n84"}}})
	}
    }
    }
    }
